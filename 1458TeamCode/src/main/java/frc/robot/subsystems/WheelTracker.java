...
public class WheelTracker {
	...
	private Translation2d mRobotVelocity = Translation2d.identity();
	...
	private void updateRobotPose(double timestamp) {
		...
		for (int i = 0; i < mModules.length; i++) {
			...
			double delta = w.estimatedRobotPose
					.getTranslation()
					.translateBy(mRobotPose.getTranslation().inverse())
					.norm();
			deltas[i] = delta;
			avg_delta += delta;
		}
		...
		if (sample_window > 0.02) {
			...
			mRobotVelocity = translation.scale(1.0 / sample_window);
	...
	private void resetModulePoses(Pose2d mRobotPose) {
		for (int i = 0; i < mModules.length; i++) {
			...
			Translation2d modulePosition = mRobotPose
	...
	public double wheel0_x() {
		return WheelProperties[0].position.x();
	}

	public double wheel0_y() {
		return WheelProperties[0].position.y();
	}

	public double wheel1_x() {
		return WheelProperties[1].position.x();
	}

	public double wheel1_y() {
		return WheelProperties[1].position.y();
	}

	public double wheel2_x() {
		return WheelProperties[2].position.x();
	}

	public double wheel2_y() {
		return WheelProperties[2].position.y();
	}

	public double wheel3_x() {
		return WheelProperties[3].position.x();
	}

	public double wheel3_y() {
		return WheelProperties[3].position.y();
	}

	public double robot_x() {
		return mRobotPose.getTranslation().x();
	}

	public double robot_y() {
		return mRobotPose.getTranslation().y();
	}
}
